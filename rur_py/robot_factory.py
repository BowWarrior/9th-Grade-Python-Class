...
class Robot_brain1(object):
    ...
    ####### Hross synthesized this code 23 Jan 15 #######
        dimx, dimy = self.parent.world_dimensions
        #if (gotox, gotoy) in self.parent.walls_list:
        #    mesg = _("You can't teleport me into a wall!\nAt: (" + str(gotox) + ", " + str(gotoy) + ")")
        #    raise dialogs.HitWallException(mesg)
        if (gotox, gotoy) in self.parent.borders:
            mesg = _("You can't teleport me into a wall!\nAt: (" + str(gotox) + ", " + str(gotoy) + ")")
            raise dialogs.HitWallException(mesg)
        if (dimx < gotox)|(dimy < gotoy)|((gotox | gotoy) <= 0):
            mesg = _("You can't go into the real world!!!\nAt: (" + str(gotox) + ", " + str(gotoy) + ") vs " + str(dimx) + " by " + str(dimy))
            raise dialogs.OuterWorldException(mesg)
        else:
            self._x = gotox
            self._y = gotoy
...
Please make sure that there is no wall in front of me!""") # There was a comma after "Please"... the comma splice was killing me, so I removed it. -Hross, 23 Jan 15
            raise dialogs.HitWallException(mesg)
